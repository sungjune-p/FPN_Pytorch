...
from model.utils.net_utils import _smooth_l1_loss, _smooth_l1_loss_mod, _crop_pool_layer, _affine_grid_gen, _affine_theta
import time
import pdb
...
class _FPN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, th=0):
...
#s#        roi_pool_feat = F.dropout(roi_pool_feat, p=th, training=True)
        pooled_feat = self._head_to_tail(roi_pool_feat)


        # compute bbox offset
        bbox_pred = self.RCNN_bbox_pred(pooled_feat)
        # loc_uncertain = self.Loc_Uncertain(pooled_feat)
...
#        pooled_feat = F.dropout(pooled_feat, True)
...
#        mean_loc = 0
        for ii in range(iteration):
            roi_pool_feat_epi = F.dropout(roi_pool_feat, p=0.3, training=True)
            pooled_feat_epi = self._head_to_tail_dropout(roi_pool_feat_epi.detach())
            pooled_feat_epi = F.dropout(pooled_feat_epi, p=0.3, training=True)

            cls_score_epi = self.RCNN_cls_score(pooled_feat_epi.detach())
            cls_prob_epi = F.softmax(cls_score_epi)

            mean_cls = mean_cls + cls_prob_epi

        mean_cls_epi = mean_cls / iteration
        var_cls_epi = (-1) * (mean_cls_epi * torch.log(mean_cls_epi + 1e-10))
        var_cls_epi = torch.sum(var_cls_epi, dim=1)
...
#        var_cls_epi = var_cls_epi / var_cls_epi.max()
        ...
        if self.training:
            ...
            # RCNN_loss_bbox = _smooth_l1_loss_mod(bbox_pred, rois_target, loc_uncertain, rois_inside_ws, rois_outside_ws)
        ...
        if self.training:
            ...
            return rois, cls_prob, cls_score, bbox_pred, rpn_loss_cls, rpn_loss_bbox, RCNN_loss_cls, RCNN_loss_bbox, rois_label, var_cls_epi
