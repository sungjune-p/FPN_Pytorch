...
def vis_detections(im, class_name, dets, uncer_cls, ind, thresh=0.8):
    """Visual debugging of detections."""
    for i in range(np.minimum(20, dets.shape[0])):
        bbox = tuple(int(np.round(x)) for x in dets[i, :4])
        score = dets[i, -1]
        uncertain_cls = uncer_cls[i]
##        uncertain_cls = uncer_cls[i,ind]
#s#        uncertain_cls = cls_var[i,ind]
#s#        uncertain_loc = (loc_var[i,0] + loc_var[i,1] + loc_var[i,2] + loc_var[i,3])/4
        if score > thresh:
            cv2.rectangle(im, bbox[0:2], bbox[2:4], (0, 204, 0), 2)
            cv2.putText(im, '%s: %.2f' % (class_name, score), (bbox[0], bbox[1] + 15), cv2.FONT_HERSHEY_PLAIN, 1.0, (0, 0, 255), thickness=2)
            cv2.putText(im, 'p: %.3f' % (uncertain_cls), (bbox[0], bbox[1] + 30), cv2.FONT_HERSHEY_PLAIN, 1.0, (0, 0, 255), thickness=2) #cv2.FONT_HERSHEY_PLAIN
...
def _smooth_l1_loss_mod(bbox_pred, bbox_targets, loc_uncertain, bbox_inside_weights, bbox_outside_weights, sigma=1.0, dim=[1]):
    ...
    loc_uncer = torch.sqrt(torch.clamp(torch.exp(loc_uncertain), min=1e-18))
    in_loc_uncer = bbox_inside_weights * loc_uncer

    l2_loc_uncer = 0.5 * (torch.exp((-1) * in_loc_uncer) * torch.pow(in_box_diff,2) + in_loc_uncer) + torch.pow(in_box_diff, 2) * (sigma_2 / 2.)
    l1_loc_uncer = torch.exp((-1) * in_loc_uncer) * (abs_in_box_diff - 0.5) + (0.5 * in_loc_uncer) + abs_in_box_diff - (0.5 / sigma_2)

    in_loss_box = l2_loc_uncer * smoothL1_sign + l1_loc_uncer * (1. - smoothL1_sign)
