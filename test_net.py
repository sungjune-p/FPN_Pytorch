...
#        for ii in range(1):
        ...
        _, _, _, _, _, uncer_cls = fpn(im_data, im_info, gt_boxes, num_boxes, th=0.0)
...
#        var2_norm = torch.squeeze(var2_norm,0)
        ...
        for j in xrange(1, imdb.num_classes):
            ...
            if inds.numel() > 0:
                ...
                uncertainty_cls = uncer_cls[inds]
                uncertainty_cls = uncertainty_cls[order]
...
#                var_norm_loc = var_norm_loc[order]
                ...
                uncertainty_cls = uncertainty_cls[keep.view(-1).long()]

#                 for jjj in range(cls_dets.size(0)):
# #                  score_list.append(cls_dets.cpu().numpy()[jjj])
#                   uncer_list.append(uncertainty_cls.data.cpu().numpy()[jjj])
...
#                print (matlab_score.shape, matlab_uncer_cls.shape, matlab_uncer_loc.shape)
                if vis:
                    im2show = vis_detections(im2show, imdb.classes[j], cls_dets.cpu().numpy(), uncertainty_cls.data.cpu().numpy(), j, 0.3)
        ...
        # if vis:
        #     cv2.imwrite('./tmp/' + str(i) + '.png', im2show)
#            assert 1==0
            #pdb.set_trace()
...
#    io.savemat('./matfile/score.mat', {'score': np.array(score_list)})
#    io.savemat('./matfile/uncer.mat', {'fpn_uncer': np.array(uncer_list)})
